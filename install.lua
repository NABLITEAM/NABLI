...
local RunBot = io.open("NABLI", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/NABLI
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr NABLI.lua
wget "https://raw.githubusercontent.com/hmodinabli/NABLI/main/NABLI.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./NABLI.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S NABLI -X kill
screen -S NABLI ./NABLI
done
]])
RunTs:close()
end
